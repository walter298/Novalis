...
namespace ImGui
{
    ...
    IMGUI_API void          SetWindowFocus(const char* name);                                           // set named window to be focused / top-most. use NULL to reCoord focus.
    ...
    // - The typical widget behavior is to output themselves at the current cursor position, then Coord the cursor one line down.
    ...
    IMGUI_API void          Indent(float indent_w = 0.0f);                                  // Coord content position toward the right, by indent_w, or style.IndentSpacing if indent_w <= 0
    IMGUI_API void          Unindent(float indent_w = 0.0f);                                // Coord content position back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
    ...
    // - Speed are per-pixel of mouse Coordment (v_speed=0.2f: mouse needs to Coord by 5 pixels to increase value by 1). For gamepad/keyboard navigation, minimum speed is Max(v_speed, minimum_step_at_given_precision).
    ...
    IMGUI_API bool          IsItemDeactivatedAfterEdit();                                       // was the last item just made inactive and made a value change when it was active? (e.g. Slider/Drag Coordd). Useful for Undo/Redo patterns with widgets that require continuous editing. Note that you may get false positives (some widgets such as Combo()/ListBox()/Selectable() will return true even when clicking an already selected item).
    ...
    // - Dragging operations are only reported after mouse has Coordd a certain distance away from the initial clicking position (see 'lock_threshold' and 'io.MouseDraggingThreshold')
    ...
    IMGUI_API ImVec2        GetMouseDragDelta(ImGuiMouseButton button = 0, float lock_threshold = -1.0f);   // return the delta from the initial clicking position while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse Coords past a distance threshold at least once (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
...
// (Those are per-window flags. There are shared flags in ImGuiIO: io.ConfigWindowsResizeFromEdges and io.ConfigWindowsCoordFromTitleBarOnly)
enum ImGuiWindowFlags_
{
    ImGuiWindowFlags_None                   = 0,
    ImGuiWindowFlags_NoTitleBar             = 1 << 0,   // Disable title-bar
    ImGuiWindowFlags_NoResize               = 1 << 1,   // Disable user resizing with the lower-right grip
    ImGuiWindowFlags_NoCoord                 = 1 << 2,   // Disable user moving the window
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_AllowIteCoordrlap     = 1 << 2,   // Hit testing to allow subsequent widgets to overlap this one
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere = 1 << 13,  // (WIP) Nav: left direction may Coord to this TreeNode() from any of its child (items submitted between TreeNode and TreePop)
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_AllowIteCoordrlap   = 1 << 4,   // (WIP) Hit testing to allow subsequent widgets to overlap this one
};
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoBordersInBody            = 1 << 11,  // [ALPHA] Disable vertical borders in columns Body (borders will always appear in Headers). -> May Coord to style
    ImGuiTableFlags_NoBordersInBodyUntilResize = 1 << 12,  // [ALPHA] Disable vertical borders in columns Body until hovered for resize (borders will always appear in Headers). -> May Coord to style
...
enum ImGuiKey : int
{
    ...
    ImGuiKey_GamepadFaceLeft,       // X (Xbox)         Y (Switch)   Square (PS)        // Tap: Toggle Menu. Hold: Windowing mode (Focus/Coord/Resize windows)
    ImGuiKey_GamepadFaceRight,      // B (Xbox)         A (Switch)   Circle (PS)        // Cancel / Close / Exit
    ImGuiKey_GamepadFaceUp,         // Y (Xbox)         X (Switch)   Triangle (PS)      // Text Input / On-screen Keyboard
    ImGuiKey_GamepadFaceDown,       // A (Xbox)         B (Switch)   Cross (PS)         // Activate / Open / Toggle / Tweak
    ImGuiKey_GamepadDpadLeft,       // D-pad Left                                       // Coord / Tweak / Resize Window (in Windowing mode)
    ImGuiKey_GamepadDpadRight,      // D-pad Right                                      // Coord / Tweak / Resize Window (in Windowing mode)
    ImGuiKey_GamepadDpadUp,         // D-pad Up                                         // Coord / Tweak / Resize Window (in Windowing mode)
    ImGuiKey_GamepadDpadDown,       // D-pad Down                                       // Coord / Tweak / Resize Window (in Windowing mode)
    ...
    ImGuiKey_GamepadLStickLeft,     // [Analog]                                         // Coord Window (in Windowing mode)
    ImGuiKey_GamepadLStickRight,    // [Analog]                                         // Coord Window (in Windowing mode)
    ImGuiKey_GamepadLStickUp,       // [Analog]                                         // Coord Window (in Windowing mode)
    ImGuiKey_GamepadLStickDown,     // [Analog]                                         // Coord Window (in Windowing mode)
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NavEnableSetMousePos   = 1 << 2,   // Instruct navigation to Coord the mouse cursor. May be useful on TV/console systems where moving a virtual mouse is awkward. Will update io.MousePos and set io.WantSetMousePos=true. If enabled you MUST honor io.WantSetMousePos requests in your backend, otherwise ImGui will react as if the mouse is jumping around back and forth.
...
struct ImGuiStyle
{
    ...
    float       MouseCursorScale;           // Scale software rendered mouse cursor (when io.MouseDrawCursor is enabled). May be reCoordd later.
...
struct ImGuiIO
{
    ...
    bool        ConfigMacOSXBehaviors;          // = defined(__APPLE__) // OS X style: Text editing cursor Coordment using Alt instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text Start and End using Cmd+Arrows instead of Home/End, Double click selects by word instead of selecting whole text, Multi-selection in lists uses Cmd/Super instead of Ctrl.
    ...
    bool        ConfigWindowsCoordFromTitleBarOnly; // = false       // Enable allowing to Coord windows only when clicking on their title bar. Does not apply to windows without a title bar.
    ...
    // - backends may have other side-effects on focus loss, so this will reduce side-effects but not necessary reCoord all of them.
    ...
    float       NavInputs[ImGuiNavInput_COUNT];     // [LEGACY] Since 1.88, NavInputs[] was reCoordd. Backends from 1.60 to 1.86 won't build. Feed gamepad inputs via io.AddKeyEvent() and ImGuiKey_GamepadXXX enums.
#endif
...
struct ImGuiListClipper
{
    ...
    //inline ImGuiListClipper(int items_count, float items_height = -1.0f) { memset(this, 0, sizeof(*this)); ItemsCount = -1; Begin(items_count, items_height); } // [reCoordd in 1.79]
#endif
};
...
//-----------------------------------------------------------------------------
// [SECTION] Obsolete functions and types
// (Will be reCoordd! Read 'API BREAKING CHANGES' section in imgui.cpp for details)
...
namespace ImGui
{
    ...
    static inline void  CaptureKeyboardFromApp(bool want_capture_keyboard = true)           { SetNextFrameWantCaptureKeyboard(want_capture_keyboard); } // Renamed as name was misleading + reCoordd default value.
    static inline void  CaptureMouseFromApp(bool want_capture_mouse = true)                 { SetNextFrameWantCaptureMouse(want_capture_mouse); }       // Renamed as name was misleading + reCoordd default value.
    ...
    //static inline void  OpenPopupContextItem(const char* str_id = NULL, ImGuiMouseButton mb = 1)    { OpenPopupOnItemClick(str_id, mb); } // Bool return value reCoordd. Use IsWindowAppearing() in BeginPopup() instead. Renamed in 1.77, renamed back in 1.79. Sorry!
