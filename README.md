...
There is also a node editor GUI where users can save and create nodes by dragging and dropping polygons and textures, and uploading child nodes. Other library features include 
...
Users can upload images and click a button to instantly draw a hitbox perfectly around the contours of the image. Users can also manually draw hitboxes. 
...
Users can save nodes to JSON files from the node editor. They can also load them in from JSON files in order to continue working on them, as well as upload them as child nodes into a 
different node that they are working on. 
