...
void SpriteEditor::open(SDL_Renderer* renderer) {
	auto filePath = openFile({"nv_sprite"});
	if (filePath) {
		try {
			m_objects.clear();
...
void nv::editor::SpriteEditor::save() {
	auto filename = saveNewFile({ "nv_sprite" }, [this](auto path) {
...
void nv::editor::SpriteEditor::saveAsTextureObject() {
	...
	saveNewFile({ "nv_texture_object" }, [&, this](auto path) {
		json json;
