...
void SceneEditor::loadSprite() {
	...
	try {
		...
		auto& currSpriteLayer = std::get<EditedObjectHive<Sprite>>(m_objectLayers.layers[m_currLayer]);
...
void SceneEditor::createRect() {
	/*auto& rectLayer = std::get<EditedObjectHive<Rect>>(m_objectLayers.layers[m_currLayer]);
	auto& lastRect = rectLayer.emplace(m_renderer, 0, 0, 0, 0)->obj;
	lastRect.setPos(400, 400);
...
void SceneEditor::reduceOpacityOfOtherLayers() {
	...
	/*forEachDataMember([](auto& objs) {
		...
		return STAY_IN_LOOP;
	}, m_objectLayers.layers[m_currLayer]);*/

	auto visibleLayerIt = m_objectLayers.layers.find(m_currLayer); //call find to get iterator
	/*auto beforeVisibleLayer = ranges::subrange(m_objectLayers.layers.begin(), visibleLayerIt);
	auto afterVisibleLayer = ranges::subrange(std::next(visibleLayerIt), m_objectLayers.layers.end());
...
void SceneEditor::showFontOptions() {
	...
	if (selectedFont != nullptr) {
		/*auto& currLayer = std::get<EditedObjectHive<Text>>(m_objectLayers.layers[m_currLayer]);
...
void SceneEditor::createTextures() noexcept {
	...
	//auto& texLayer = std::get<EditedObjectHive<Texture>>(m_objectLayers.layers[m_currLayer]);
...
void nv::editor::SceneEditor::save() const noexcept {
	saveNewFile({ "nv_scene" }, [this](auto path) {
		json root;
		...
		for (const auto& [layer, objs] : m_objectLayers.layers) {
...
void nv::editor::SceneEditor::cameraMove(int dx, int dy) {
	m_objectLayers.forEach([&](int layer, auto& obj) {
		//obj.obj.move(dx, dy);
		return STAY_IN_LOOP;
	});
}
...
void nv::editor::SceneEditor::editLayers() {
	...
	//selectSpecialPoint();
	//m_objectSelection(m_objectLayers);
}
...
void nv::editor::SceneEditor::sdlRender() const noexcept {
	m_objectLayers.forEach([](int layer, const auto& obj) {
		//obj.obj.render();
		return STAY_IN_LOOP;
	});
