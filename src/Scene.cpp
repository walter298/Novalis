...
//#include "data_util/SceneObjectJsonSerialization.h"
//
...
//			m_mouseData.x = static_cast<float>(m_SDLEvt.button.x);
//			m_mouseData.y = static_cast<float>(m_SDLEvt.button.y);
//			deltaX = static_cast<float>(m_SDLEvt.motion.x);
//			deltaY = static_cast<float>(m_SDLEvt.motion.y);
//			break;
//		case SDL_TEXTINPUT:
...
//		}
//	}
//	m_mouseData.deltaX = static_cast<float>(deltaX);
//	m_mouseData.deltaY = static_cast<float>(deltaY);
...
//
//		sprites  = objsJson[typeid(Sprite).name()].get<plf::hive<Sprite>>();
//		textures = objsJson[typeid(Texture).name()].get<plf::hive<Texture>>();
//		text     = objsJson[typeid(Text).name()].get<plf::hive<Text>>();
//		rects    = objsJson[typeid(Rect).name()].get<plf::hive<Rect>>();
//	};
//
...
//		m_specialPoints.emplace(pointJson["name"].get<std::string>(), pointJson["point"].get<SDL_FPoint>());
//	}
//	sceneFile.close();
//}
//
