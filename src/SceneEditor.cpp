...
void nv::editor::SceneEditor::createTexture(Renderer& renderer) noexcept {
	auto filename = openFilePath();
	if (filename) {
		auto& texLayer = m_localTextureLayers[m_currLayer];
		texLayer.emplace(*filename, IMG_LoadTexture(renderer.get(), filename->c_str()), TextureData{});
...
void SceneEditor::showRightClickOptions(Renderer& renderer) noexcept {
...
nv::editor::EditorDest SceneEditor::operator()(Renderer& renderer) {
	...
	if (m_showingRightClickOptions) {
		showRightClickOptions(renderer);
	}
	moveCamera(renderer);
