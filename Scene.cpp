...
nv::Scene::Scene(nv::Instance& instance) 
	: m_renderer{instance.getRawRenderer() } {
}
...
void nv::Scene::addObj(Sprite obj, int layer) noexcept {
	m_renderer.addObj(std::move(obj), obj.getID(), layer);
}

void nv::Scene::addObj(Text obj, int layer) noexcept {
	m_texts.push_back(std::move(obj));
	m_renderer.addObj(&m_sprites.back(), obj.getID(), layer);
}

void nv::Scene::stopRendering(const RenderObjID& ID, int layer) noexcept {
	m_renderer.reCoordObj(ID, layer);
}
...
void nv::Scene::reCoordButton(ID<Button> ID) noexcept {
...
void nv::Scene::camCoord(int dx, int dy) {
	//todo
}
