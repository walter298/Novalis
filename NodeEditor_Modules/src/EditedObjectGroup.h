...
namespace nv {
	namespace editor {
		...
		private:
			...
			using MultiPolygon = bg::model::multi_polygon<BGPolygon>;

			/*void addPolygonOutline(MultiPolygon& multiPolygon, EditedObjectData<DynamicPolygon>& polygon) {
				BGPolygon bgPolygon;
				bg::assign_points(bgPolygon, polygon.obj.points);
				multiPolygon.push_back(bgPolygon);
			}*/

			/*static Point getCentralPoint(const EditedObjectGroup::Objects& objects) {
				
				MultiPolygon multiPolygon;

				Point centroid;

				nv::detail::forEachDataMember([&]<typename Object>(EditedObjectData<Object>* objects) {
					for (auto& object : objects) {
						auto& polygon = object->obj;
						BGPolygon bgPolygon;
						bg::assign_points(bgPolygon, polygon.points);
						multiPolygon.push_back(bgPolygon);
					}
					return nv::detail::STAY_IN_LOOP;
				}, objects);
