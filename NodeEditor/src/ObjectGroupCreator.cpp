...
void nv::editor::ObjectGroupCreator::showObjectSearchChild(NameManager& objectGroupNameManager) {
...
bool nv::editor::ObjectGroupCreator::showCurrentStoredObjectsChild(ObjectGroupManager& objectGroupManager) {
	ImGui::SameLine();
	...
	if (ImGui::Button("Create Object Group")) {
		auto groupID = objectGroupManager.addGroup(m_objectGroupName);
		for (auto& object : m_currentlyStoredObjects.objects) {
			std::visit([&](auto& objectRef) {
				objectGroupManager.addObjectToGroup(groupID, objectRef.get());
			}, object);
		}
...
bool nv::editor::ObjectGroupCreator::show(ObjectGroupManager& objectGroupManager, NameManager& objectGroupNameManager) {
	bool showing = true;

	centerNextWindow();
	ImGui::SetNextWindowSize({ 1400.0f, 700.0f });
	ImGui::Begin(OBJECT_GROUP_CREATION_WINDOW_NAME, &showing, DEFAULT_WINDOW_FLAGS);
