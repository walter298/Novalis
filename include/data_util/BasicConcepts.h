...
namespace nv {
	...
	namespace concepts {
		...
		concept ScaleableObject = requires(Object obj) {
			{ obj } -> RenderableObject;
			obj.scale(0, 0);
			obj.scale(SDL_FPoint{});
		};

		template<typename Object>
		concept MoveableObject = requires(Object obj) {
			{ obj.move(SDL_FPoint{}) } -> std::same_as<void>;
			{ obj.screenMove(SDL_FPoint{}) } -> std::same_as<void>;
			{ obj.worldMove(SDL_FPoint{}) } -> std::same_as<void>;
			{ obj.setScreenPos(SDL_FPoint{}) } -> std::same_as<void>;
			{ obj.setWorldPos(SDL_FPoint{}) } -> std::same_as<void>;
			{ obj.getScreenPos() } -> std::same_as<SDL_FPoint>;
			{ obj.getWorldPos() } -> std::same_as<SDL_FPoint>;
		};

		template<typename Object>
		concept SizeableObject = requires(Object obj) {
			obj.setSize(500, 500);
			obj.setSize(SDL_FPoint{ 500, 500 });
			{ obj.getSize() } -> std::same_as<SDL_FPoint>;
		};
